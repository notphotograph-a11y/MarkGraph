/**
 * wikilink / 标签解析与路径解析（F23.1 索引下沉后的唯一实现）。
 * 编辑器装饰、[[ 补全、断链检测（前端）、链接索引（服务端 /api/index 与 MCP）
 * 与 AI 建议链接全部引用本模块——任何解析规则变更两端同时生效。
 * 纯函数、无 node/DOM 依赖，前端经 src/editor/wikilink.ts 桶文件引用。
 */
...
/** 正文里已链接的目标名集合（小写，用于过滤重复建议与重命名改写范围判断） */
...
/**
 * 重命名后改写全库 wikilink（F23.2）：
 * mapFn 收到原目标文本（保留大小写，不带 .md），返回新目标或 null（不改）。
 * 只改目标部分，heading/alias 原样保留；返回 [新正文, 是否有改动]。
 */
export function rewriteLinks(
  body: string,
  mapFn: (target: string) => string | null,
): [string, boolean] {
  let changed = false
  WIKILINK_RE.lastIndex = 0
  const out = body.replace(WIKILINK_RE, (_full, inner: string) => {
    const parsed = parseLink(inner)
    const next = mapFn(parsed.target.replace(/\.md$/i, ''))
