/**
 * 链接索引器（docs/02 §5.3 / F23.1 索引下沉）：输入「全部笔记路径+内容」的 Map，
 * 输出 nodes / edges / backlinks(含上下文摘录) / tags。
 * 服务端 /api/index 与 MCP 查询工具直接消费；前端经 src/graph/indexer.ts 桶文件
 * 引用同一实现（断链弹窗的「创建此笔记」路径推导也依赖 ghostTargets）。
 */
import {
  TAG_RE,
  WIKILINK_RE,
...
import { fmTags, splitFrontmatter } from './frontmatter.js'
