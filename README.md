...
- **低负担操作**：删除进回收站（30 天可恢复）、改名自动改写全库双链、新建笔记免命名一键直开、⌘S 立即保存、模板与今日日记、标签视图、搜索命中直达原文位置（⌘P 即搜即达 + 最近打开）
...
- **安装为应用（PWA）**：手机「添加到主屏幕」、桌面「安装」后以独立窗口打开——应用壳自动缓存，断连时明确提示不假装可用；数据仍只在服务器，无离线副本
...
4. **多端客户端**（原生 / 壳应用）——REST + SSE API 已为其就绪；第一步 PWA 应用壳已落地，原生壳按需另议
...
多端访问的轻量形态：手机/平板「添加到主屏幕」，桌面 Chrome/Edge「安装」——MarkGraph 以独立窗口打开（无浏览器地址栏），带应用图标。数据仍全部在你的服务器上，**浏览器没有离线副本**；断网或服务不可达时界面会明确提示，恢复后自动继续。

- 前提：通过 `https://` 或 `http://127.0.0.1` 访问（浏览器规定，Service Worker 只在安全上下文注册）。走 NAS 反代/Tailscale HTTPS 的部署开箱即得；纯 http + 局域网 IP 时自动退化为普通网页，功能不受影响
- 版本更新自动完成：服务端发新版后，下次打开即是新版，无需手动清缓存
- 改应用图标：编辑 `scripts/app-icon.svg` 后跑 `node scripts/gen-icons.mjs`
